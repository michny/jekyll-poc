...
//This is required so it can render the liquid based for loop as an actual for loop rather than just writing the code as text
:page-liquid:
...
//++++ means that the content should be treated as raw html (the a href) rather than text
++++
{% for post in site.posts limit: 5 %}
<a href=".{{ post.url }}">{{ post.navtitle }}</a>
{% endfor %}
